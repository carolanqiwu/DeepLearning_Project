...
chrom_size = pd.read_csv(f'/data1/lesliec/carolw/genome/hg38/{GENOME_ASSEMBLY}.chrom.sizes', #downloaded from UCSC
...
def atac_processing(frag_path, nrows_skip, valid_barcode_path,
    ...
    # debug for dimensionality mismatch b/w scatac_dict.p and metacell
    #valid_barcode = [str.split(i,"#")[1] for i in valid_barcode.to_list()]
    valid_barcode = valid_barcode.astype(str)
    print("valid barcode count before filter:", len(valid_barcode))
    #frag[3] = frag[3].apply(lambda x: x.split("#")[1])
    frag[3] = frag[3].astype(str)
    print("frag barcode count before filter:", len(frag))
    frag = frag[frag[3].isin(valid_barcode)]
    print("frag barcode count after filter:", len(frag))
    ...
    # ensure chr prefix
    frag[0] = frag[0].astype(str)
    frag[0] = frag[0].apply(lambda x: x if x.startswith("chr") else "chr" + x)
    ...
    tile_dict = {}
    print("Fragments chromosomes:", set(frag_stacked[0]))
    print("Valid chromosomes:", valid_chrom)
...
def metacell_computing(lsi_path, n_neighbors = 100, max_overlap = 33,
    ...
    tile_barcodes = np.load(
    	f"{save_path}/atac/{cell_type_prefix}_tile_500bp_barcode.npy",
    	allow_pickle=True
    )
    ...
    lsi.index = lsi.index.astype(str)
    # HARD alignment
    lsi = lsi.loc[tile_barcodes]
...
def main():
    ...
    chrom_size = pd.read_csv(f'/data1/lesliec/carolw/genome/hg38/{GENOME_ASSEMBLY}.chrom.sizes', 
                         sep = '\t', header = None, index_col = 0)
