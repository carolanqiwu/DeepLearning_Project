...
def get_metacell_profile(sc_atac_dict, nbrs):
    ...
    metacell = nbrs
    print("metacell (nbrs) shape:", metacell.shape)
    for chrom in list(sc_atac_dict.keys()):
        mat = sc_atac_dict[chrom]
        print(f"{chrom}: scATAC shape =", mat.shape)

        # HARD assertion — fail early, fail loud
        assert metacell.shape[1] == mat.shape[0], (
            f"Dimension mismatch on {chrom}: "
            f"metacell expects {metacell.shape[1]} cells, "
            f"but scATAC has {mat.shape[0]}"
        )
